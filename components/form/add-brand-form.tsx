...
import { addBrand } from "@/app/redux/features/brands/brandsSlice";
...
export function AddBrandForm({ onClose }: { onClose: () => void }) {
  const dispatch = useAppDispatch();
  const loading = useAppSelector((state: RootState) => state.brands.loading_add);
  const error = useAppSelector((state: RootState) => state.brands.error_add);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const action = await dispatch(addBrand({ title, image: urlToUse }));

      if (!title) {
        toast.success("The brand has been successfully added.");
      }
      ...
      if (addBrand.fulfilled.match(action)) {
        ...
        console.error("Failed to add brand:", action.error);
      }
    } catch (err) {
      console.error("Error creating brand:", err);
      toast.error("Failed to create the brand. Please try again.");
    } finally {
      onClose();
    }
  };
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Fill out the form to add a new brand to your store.
        </DialogDescription>
      </DialogHeader>
