...
import { updateBrand } from "@/app/redux/features/brands/brandsSlice"; 
...
export function EditBrandForm({ brandId, currentTitle, currentImage, onClose }: { brandId: string, currentTitle: string, currentImage: string, onClose: () => void }) {
  ...
  const loading = useAppSelector((state: RootState) => state.brands.loading_update);
  const error = useAppSelector((state: RootState) => state.brands.error_update);
  ...
  const uploadImage = async () => {
    ...
    const storageRef = ref(storage, `brands/${newFileName}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultAction = await dispatch(updateBrand({ brandId, title, image: uploadedImageUrl }));

      if (updateBrand.fulfilled.match(resultAction)) {
        toast.success("The brand has been successfully updated.");
        resetForm();
      } else {
        toast.error("Failed to update the brand. Please try again.");
      }
    } catch (error) {
      console.error(error)
      toast.error("Failed to update the brand. Please try again.");
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>Update the details of the brand.</DialogDescription>
      </DialogHeader>
